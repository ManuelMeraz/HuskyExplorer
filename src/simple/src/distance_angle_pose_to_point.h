...
int map_index(const map_inf &map, const int &row, const int &column); 
#define MAP_INDEX(map, i, j) ((i) + (j) * map.size_x)

// Converts grid values based off map resolution to a pose in meters
double grid_to_pose_x(const map_inv &map, const int &grid_x);
double grid_to_pose_y(const map_inv &map, const int &grid_y);
